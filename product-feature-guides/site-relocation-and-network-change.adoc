...
.Pasos
. Si en la nueva ubicación se va a utilizar una nueva subred de Grid, link:https://docs.netapp.com/us-en/storagegrid/expand/updating-subnets-for-grid-network.html["Agregue la subred a la lista de subred de red de cuadrícula"^]
...
. Conéctate al appliance mediante ssh y apaga el nodo usando 'shutdown -h now'
