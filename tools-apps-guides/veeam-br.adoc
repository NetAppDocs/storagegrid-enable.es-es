...
Asegúrese de leer la documentación del producto para https://www.veeam.com/documentation-guides-datasheets.html?productId=8&version=product%3A8%2F221["Veeam Backup and Replication"^] y https://docs.netapp.com/us-en/storagegrid/["StorageGRID"^] Antes de empezar.  Veeam proporciona calculadoras para comprender el dimensionamiento de la infraestructura de Veeam y los requisitos de capacidad que se deben utilizar antes de dimensionar su solución StorageGRID .  Consulte siempre las configuraciones validadas de Veeam- NetApp en el sitio web del Programa Veeam Ready. https://www.veeam.com/alliance-partner-technical-programs.html?alliancePartner=netapp1&page=1["Veeam Ready Object, inmutabilidad de objetos y repositorio"^] .
...
. Abra el Administrador de cuadrícula StorageGRID .
. En la esquina superior derecha, seleccione el signo de interrogación y luego Documentación API.
. En la esquina superior derecha, haga clic en el enlace de la página Documentación de API privada.
. Expandir ilm-advanced.
. Seleccione OBTENER ilm-advanced.
. Seleccione Probar y luego Ejecutar.
. Compruebe el resultado de la respuesta.
+
.. Si los valores son nulos, significa que se utilizan los valores predeterminados de ilm-advanced.
.. Si los valores no son nulos, significa que se están utilizando valores avanzados de ILM personalizados.  Copia toda la salida después de "data" :, comenzando con { hasta el segundo al último }.
+
... Guárdalo en algún editor de texto.
+
Ejemplo de respuesta:

+
image:veeam-bp/get_ilm_adv_before.png["OBTENER salida ILM-Advanced"]





. Seleccione PUT ilm-advanced.
. Seleccione Probarlo para comenzar a editar el cuerpo de la API.
+
.. De forma predeterminada, el cuerpo de la API contendrá valores predeterminados y no valores personalizados que se hayan configurado previamente.  Esta es la razón por la que es MUY importante ejecutar los pasos 5 a 7.


. Si se encuentran valores no predeterminados en el paso 5-7, reemplace el cuerpo de la API con la salida guardada en el paso 7. .  De lo contrario, si los valores fueron nulos en el paso 5-7, deje el cuerpo de la API como está.
. Ajuste los siguientes parámetros en el cuadro del cuerpo de la API:
+
.. Establezca el valor sincrónico en falso.
+
Ejemplo de texto del cuerpo de la API:

+
image:veeam-bp/put_ilm_adv.png["PUT ILM-Entrada avanzada"]



. Una vez completado, seleccione Ejecutar
...
* link:https://docs.netapp.com/us-en/storagegrid/["Documentación del producto NetApp StorageGRID"^]
