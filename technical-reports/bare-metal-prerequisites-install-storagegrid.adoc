...
NOTE: Las cifras de la tabla corresponden a cada tipo de servicio de StorageGRID, no a toda la grid ni a cada host físico. En función de las opciones de implementación, calcularemos las cifras para cada host físico en link:bare-metal-prerequisites-install-storagegrid.html#physical-host-layout-and-requirements["La distribución y los requisitos del host físico"], más adelante en este documento. ++ ++ Las rutas o sistemas de archivos marcados con un asterisco serán creados en el propio contenedor de StorageGRID por el instalador. No es necesario que el administrador realice ninguna configuración manual ni cree ningún sistema de archivos, pero los hosts necesitan dispositivos de bloque para cumplir con estos requisitos. En otras palabras, el dispositivo de bloque debería aparecer usando el comando `lsblk` pero no debe estar formateado ni montado en el sistema operativo del host. ++ ++
...
|===
En este ejemplo, se necesitan los tamaños de disco que se indican en la siguiente tabla para cada tipo de contenedor. Los requisitos por host físico se describen en link:bare-metal-prerequisites-install-storagegrid.html#physical-host-layout-and-requirements["La distribución y los requisitos del host físico"], más adelante en este documento.
...
Si no utilizas almacenamiento iSCSI, debes asegurarte de que host1, host2 y host3 tengan dispositivos de bloque con un tamaño suficiente para cumplir con sus requisitos. Consulta link:bare-metal-prerequisites-install-storagegrid.html#disk-sizes-per-container-type["Tamaños de disco por tipo de contenedor"] para ver los requisitos de almacenamiento de host1, host2 y host3.
...
----
. Utilizando el nombre del iniciador del paso 2, asigna los LUN en tu dispositivo de almacenamiento (del número y tamaño que se muestran en la link:bare-metal-prerequisites-install-storagegrid.html#storage-requirements["Requisitos de almacenamiento"] tabla) a cada nodo de almacenamiento.
